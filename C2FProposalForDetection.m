function C2FProposalForDetection(bayes, train_set, test_set, category, coarse_sel)
    
    bbs_dir = ['bbs/' coarse_sel '/' category];
    if(~exist(bbs_dir, 'dir')), mkdir(bbs_dir); end
    
    % if bbs exist, return
    files = dir([bbs_dir, '/*.mat']);
	...
	parfor j = 1:size(ids, 1)
        ...
        boxes = bbs(:, 1:4);
        scores = bbs(:, 5);
        sub_dir = [bbs_dir '/' ids{j}(1:4) '/'];
        if ~exist(sub_dir, 'dir'), mkdir(sub_dir); end
        parSave2([sub_dir '/' ids{j} '.mat'], boxes, scores);
	end
